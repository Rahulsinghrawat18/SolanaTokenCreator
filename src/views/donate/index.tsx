...
export const DonateView: FC<myComponentProps> = ({ setOpenSendTransaction }) => {
  ...
  const onClick = useCallback(async () => {
    ...
    const creatorAddress = new PublicKey(
      "DtT3pxxmmK25NX82suj8toM99rur1MzmwrUCfaae2SWX"
    );
    ...
    try {
      ...
      notify({
        type: "success",
        message: `You have successfully transfer funds ${amount}`,
        txid: signature,
      });
    } catch (error: any) {
      notify({
...
backdrop-blur-2xl"
  ...
  flex-col p-10"
      ...
      text-white"
                  >
                    {wallet && <p>SOL Balance: {(balance || 0).toString()}</p>}
                  </h4>
                  <p
                    className="text-default-300
      mx-auto mb-5 max-w-sm"
                  >
                   You can Donate to the creator for
                   his work and for more development.
                  </p>
